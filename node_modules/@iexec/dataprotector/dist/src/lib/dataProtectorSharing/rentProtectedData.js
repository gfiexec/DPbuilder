...
import { addressOrEnsSchema, positiveNumberSchema, positiveStrictIntegerStringSchema, throwIfMissing, } from '../../utils/validators.js';
...
import { onlyAccountWithMinimumBalance, onlyProtectedDataCurrentlyForRent, onlyValidRentingParams, } from './smartContract/preflightChecks.js';
import { getProtectedDataDetails } from './smartContract/sharingContract.reads.js';
export const rentProtectedData = async ({ iexec = throwIfMissing(), sharingContractAddress = throwIfMissing(), protectedData, price, duration, }) => {
    ...
    onlyValidRentingParams({ price, duration }, protectedDataDetails.rentingParams);
    ...
    try {
        ...
        let tx;
        const rentProtectedDataCallParams = [vProtectedData, { price: vPrice, duration: vDuration }];
        if (accountDetails.sharingContractAllowance >= BigInt(vPrice)) {
            tx = await sharingContract.rentProtectedData(...rentProtectedDataCallParams, txOptions);
        }
        else {
            const callData = sharingContract.interface.encodeFunctionData('rentProtectedData', rentProtectedDataCallParams);
...
//# sourceMappingURL=rentProtectedData.js.map