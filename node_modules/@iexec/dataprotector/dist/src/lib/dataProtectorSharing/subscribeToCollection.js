...
import { onlyAccountWithMinimumBalance, onlyCollectionAvailableForSubscription, onlyValidSubscriptionParams, } from './smartContract/preflightChecks.js';
import { getCollectionDetails } from './smartContract/sharingContract.reads.js';
export const subscribeToCollection = async ({ iexec = throwIfMissing(), sharingContractAddress = throwIfMissing(), collectionId, price, duration, }) => {
    ...
    onlyValidSubscriptionParams({ price, duration }, collectionDetails.subscriptionParams);
    ...
    try {
        ...
        let tx;
        const subscribeToCollectionCallParams = [vCollectionId, { duration: vDuration, price: vPrice }];
        if (accountDetails.sharingContractAllowance >= BigInt(vPrice)) {
            tx = await sharingContract.subscribeToCollection(...subscribeToCollectionCallParams, txOptions);
        }
        else {
            const callData = sharingContract.interface.encodeFunctionData('subscribeToCollection', subscribeToCollectionCallParams);
...
//# sourceMappingURL=subscribeToCollection.js.map