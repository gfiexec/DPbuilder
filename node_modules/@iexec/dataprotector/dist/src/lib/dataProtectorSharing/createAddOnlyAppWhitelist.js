...
export const createAddOnlyAppWhitelist = async ({ iexec = throwIfMissing(), sharingContractAddress = throwIfMissing(), }) => {
    const appWhitelistRegistryContract = await getAppWhitelistRegistryContract(iexec, sharingContractAddress);
    try {
        let userAddress = await iexec.wallet.getAddress();
        userAddress = userAddress.toLowerCase();
        const { txOptions } = await iexec.config.resolveContractsClient();
        const tx = await appWhitelistRegistryContract.createAddOnlyAppWhitelist(userAddress, txOptions);
...
//# sourceMappingURL=createAddOnlyAppWhitelist.js.map