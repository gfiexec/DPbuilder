...
export const getGrantedAccess = async ({ iexec = throwIfMissing(), protectedData, authorizedApp, authorizedUser, isUserStrict = false, page, pageSize, }) => {
    ...
    try {
        const { count, orders } = await iexec.orderbook.fetchDatasetOrderbook(vProtectedData || 'any', {
        ...
        const grantedAccess = orders?.map((order) => formatGrantedAccess(order.order));
