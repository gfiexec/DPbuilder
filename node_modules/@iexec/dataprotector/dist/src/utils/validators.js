...
const isPositiveIntegerStringTest = (value) => /^\d+$/.test(value);
const isZeroStringTest = (value) => value === '0';
export const booleanSchema = () => boolean().strict().typeError('${path} should be a boolean');
export const stringSchema = () => string().strict().typeError('${path} should be a string');
export const urlSchema = () => string().matches(/^http[s]?:\/\//, '${path} should be a url');
export const taskIdSchema = () => string().matches(/^0x[a-fA-F0-9]{64}$/, '${path} must be a valid iExec task ID');
...
export const positiveIntegerStringSchema = () => string().test('is-positive-int', '${path} should be a positive integer', (value) => isUndefined(value) || isPositiveIntegerStringTest(value));
export const positiveNumberSchema = () => number().integer().min(0).typeError('${path} must be a non-negative number');
export const numberBetweenSchema = (min, max) => number()
    .integer()
    .min(min)
    .max(max)
    .typeError(`$\{path} must be a number between ${min} and ${max}`);
export const positiveStrictIntegerStringSchema = () => string().test('is-positive-strict-int', '${path} should be a strictly positive integer', (value) => isUndefined(value) ||
    (!isZeroStringTest(value) && isPositiveIntegerStringTest(value)));
export const grantedAccessSchema = () => object({
    ...
    .noUnknown()
    .default(undefined);
export const urlArraySchema = () => array().of(string().matches(/^http[s]?:\/\//, '${value} should be a url'));
export const secretsSchema = () => object().test('is-valid-secret', ({ label }) => `${label} must be an object with numeric keys and string values`, (value) => {
...
export const validateOnStatusUpdateCallback = (value = () => { }) => {
